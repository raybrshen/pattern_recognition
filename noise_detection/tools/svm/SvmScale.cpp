...
void SvmScale::scale_exit_with_help()
{
	printf(
...
int SvmScale::scale_main(int argc,char **argv)
{
	int i,index;
...
char* SvmScale::scale_readline(FILE *input)
{
	int len;
...
void SvmScale::scale_save(const char* _training)
{
    char range_name[255];
    char rawdata_name[255];
    char scaledata_name[255];
    sprintf(range_name, "%s%s", _training, ".range");
    sprintf(rawdata_name, "%s%s", _training, ".rawdata");
    sprintf(scaledata_name, "%s%s", _training, ".scale");

    int scale_argc = 8;
    char* scale_argv[] = {"ScaleSvm", "-l", "-1", "-u", "1", "-s", range_name, rawdata_name};

    int stdout_bk = dup(STDOUT_FILENO);
    FILE* fp = freopen(scaledata_name, "w", stdout);

    scale_main(scale_argc, scale_argv);

    fclose(fp);
    stdout = fdopen(stdout_bk, "w");
}

void SvmScale::scale_restore(const char* _testing, const char* _training)
{
    char range_name[255], rawdata_name[255], scaledata_name[255];
    memset(range_name, 0, sizeof(range_name));
    memset(rawdata_name, 0, sizeof(rawdata_name));
    memset(scaledata_name, 0, sizeof(scaledata_name));
    sprintf(range_name, "%s%s", _training, ".range");
    sprintf(rawdata_name, "%s%s", _testing, ".rawdata");
    sprintf(scaledata_name, "%s%s", _testing, ".scale");

    int scale_argc = 4;
    char* scale_argv[] = {"ScaleSvm", "-r", range_name, rawdata_name};
